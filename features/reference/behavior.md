...
| **Participant flights** (Trips list **Flights**): staff-only; incomplete/applied/approved only; flight header purchased/cost/comments; segments with airport/airline catalogs; board shows who purchases from application (and org preference text), plus initial departure and final arrival from min/max segment number; **application** collects flight purchase option (self vs org arranges) and, when org, preferred airports / class / airline | `GET`/`PUT .../flights`, segments dialog, apply/edit application, `airport` / `airline` / `tripFlight` / `tripFlightSegment` | Feature 30 |
